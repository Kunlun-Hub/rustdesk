...
import { AuthProviderType, type IdentityProvider } from '@prisma/client';
...
function resolveReturnUrl(value: unknown) {
...
function redirectWithParams(res: Response, returnUrl: string, params: Record<string, string>) {
  const url = new URL(returnUrl);
  Object.entries(params).forEach(([key, value]) => url.searchParams.set(key, value));
  res.redirect(url.href);
}

function externalAuthorizationCode(provider: Pick<IdentityProvider, 'type'>, query: Record<string, unknown>) {
  const candidates = provider.type === AuthProviderType.OIDC
    ? ['code']
    : ['code', 'authCode', 'auth_code', 'tmp_auth_code'];
  for (const key of candidates) {
    const value = query[key];
    if (typeof value === 'string' && value.trim()) return { code: value, field: key };
  }
  return { code: '', field: null };
}

function identityProviderMissingFields(provider: Pick<IdentityProvider, 'type' | 'issuerUrl' | 'clientId' | 'corpId' | 'agentId' | 'appKey' | 'appSecret'>) {
...
function loginOption(provider: Pick<IdentityProvider, 'id' | 'type' | 'name'>) {
