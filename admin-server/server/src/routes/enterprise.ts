...
function canReadAddressBookWhere(userId: string | undefined, isAdmin = false): Prisma.AddressBookWhereInput {
  if (isAdmin) return {};
  return {
    OR: [
...
function canWriteAddressBookWhere(userId: string | undefined, isAdmin = false): Prisma.AddressBookWhereInput {
  if (isAdmin) return {};
  return {
    OR: [
      ...(userId
...
async function requireAddressBookAccess(req: Request, res: Response, mode: 'read' | 'write') {
...
async function findAddressBookUser(userId: string | null | undefined) {
...
async function unknownAddressBookTags(addressBookId: string, tagNames: string[]) {
...
type IdentityProviderConfigInput = {
  type: 'LOCAL' | 'OIDC' | 'WECOM' | 'DINGTALK';
...
type IdentityProviderCreateData = z.infer<typeof identityProviderSchema>;
type IdentityProviderUpdateData = Partial<IdentityProviderCreateData>;

function validateIdentityProviderConfig(data: IdentityProviderConfigInput, partial = false) {
  if (!data.enabled && partial) return null;
...
function cleanIdentityProviderFields<T extends IdentityProviderUpdateData>(data: T, type: IdentityProviderConfigInput['type'], typeChanged: boolean): T {
  ...
  if (type === 'WECOM') {
    ...
    normalized.appKey = null;
    if (typeChanged && normalized.appSecret === undefined) normalized.appSecret = null;
  }
  if (type === 'DINGTALK') {
    normalized.issuerUrl = null;
    normalized.clientId = null;
...
function normalizeIdentityProviderCreateData(data: IdentityProviderCreateData): IdentityProviderCreateData {
  return cleanIdentityProviderFields(data, data.type, false);
}

function normalizeIdentityProviderUpdateData(data: IdentityProviderUpdateData, existing: IdentityProviderConfigInput): IdentityProviderUpdateData {
  const type = data.type ?? existing.type;
  return cleanIdentityProviderFields(data, type, Boolean(data.type && data.type !== existing.type));
}

function encryptIdentityProviderSecretFields<T extends IdentityProviderUpdateData>(data: T): T {
  return {
    ...data,
...
function serializeIdentityProvider<T extends {
  id: string;
  type: 'LOCAL' | 'OIDC' | 'WECOM' | 'DINGTALK';
  issuerUrl?: string | null;
  clientId?: string | null;
...
function changedSummaryFields(before: Record<string, unknown>, after: Record<string, unknown>) {
  return Object.keys(after).filter((key) => JSON.stringify(before[key]) !== JSON.stringify(after[key])).sort();
}

async function wouldRemoveLastAdmin(userId: string, next: { isAdmin?: boolean; status?: 'DISABLED' | 'NORMAL' | 'UNVERIFIED' }) {
  const current = await prisma.user.findUnique({ where: { id: userId }, select: { isAdmin: true, status: true } });
  if (!current?.isAdmin || current.status !== 'NORMAL') return false;
  const willRemainActiveAdmin = next.isAdmin !== false && (next.status === undefined || next.status === 'NORMAL');
  if (willRemainActiveAdmin) return false;
  const activeAdmins = await prisma.user.count({ where: { isAdmin: true, status: 'NORMAL' } });
  return activeAdmins <= 1;
}

async function wouldBulkRemoveAllAdmins(userIds: string[], next: { status?: 'DISABLED' | 'NORMAL' | 'UNVERIFIED' }) {
  if (!next.status || next.status === 'NORMAL') return false;
...
function wouldDemoteCurrentUser(targetUserId: string, currentUserId: string | undefined, next: { isAdmin?: boolean; status?: 'DISABLED' | 'NORMAL' | 'UNVERIFIED' }) {
...
async function requireMutableRole(roleId: string, res: Response) {
...
function diffStringSet(before: string[], after: string[]) {
...
function roleIdsFromUser(user: { roles?: Array<{ roleId?: string; role?: { id?: string } }> }) {
  return (user.roles ?? [])
    .map((item) => item.roleId ?? item.role?.id)
    .filter((roleId): roleId is string => Boolean(roleId));
}

function permissionKeysFromRole(role: { permissions?: Array<{ permission?: { key?: string } }> }) {
  return (role.permissions ?? [])
    .map((item) => item.permission?.key)
    .filter((key): key is string => Boolean(key));
}

async function unknownPermissionKeys(permissionKeys: string[]) {
...
async function unknownRoleIds(roleIds: string[]) {
...
async function unknownUserIds(userIds: string[]) {
...
async function findDuplicateRoleName(name: string | undefined, exceptId?: string) {
  if (!name) return null;
...
async function findDuplicateUserIdentity(input: { username?: string; email?: string | null }, exceptId?: string) {
  ...
  for (const user of duplicates) {
    if (input.username && user.username === input.username) fields.add('username');
    if (input.email && user.email === input.email) fields.add('email');
  }
  return [...fields].sort();
}

function userWhere(query: Request['query']): Prisma.UserWhereInput {
  ...
  return {
    ...(status ? { status: status as 'DISABLED' | 'NORMAL' | 'UNVERIFIED' } : {}),
...
type StrategyReceiptSummary = { total: number; PENDING: number; APPLIED: number; FAILED: number };

function strategyWhere(query: Request['query']): Prisma.StrategyWhereInput {
  const q = typeof query.q === 'string' ? query.q : undefined;
...
async function strategyReceiptSummary(strategyIds: string[]) {
  const receiptGroups = strategyIds.length > 0
    ...
    : [];
  const summaryByStrategy = new Map<string, StrategyReceiptSummary>();
  for (const group of receiptGroups) {
    if (!group.strategyId) continue;
...
function strategySummaryMatches(summary: StrategyReceiptSummary, rollout: string | undefined) {
  if (!rollout) return true;
  if (rollout === 'noReceipts') return summary.total === 0;
  if (rollout === 'pending') return summary.PENDING > 0;
  if (rollout === 'failed') return summary.FAILED > 0;
  if (rollout === 'applied') return summary.total > 0 && summary.APPLIED === summary.total;
  return true;
}

async function findDuplicateStrategyName(name: string | undefined, exceptId?: string) {
  if (!name) return null;
...
router.post('/strategies', requirePermission('strategies.write'), async (req, res) => {
  ...
  const strategy = await prisma.strategy.create({
    data: {
      ...parsed.data,
      configOptions: parsed.data.configOptions as Prisma.InputJsonValue,
      extra: parsed.data.extra as Prisma.InputJsonValue
    }
  });
...
router.patch('/strategies/:id', requirePermission('strategies.write'), async (req, res) => {
  ...
  const strategy = await prisma.strategy.update({
    ...
    data: {
      ...parsed.data,
      configOptions: parsed.data.configOptions as Prisma.InputJsonValue | undefined,
      extra: parsed.data.extra as Prisma.InputJsonValue | undefined,
...
router.post('/strategies/:id/repush', requirePermission('strategies.write'), async (req, res) => {
  ...
  const devicesById = new Map<string, { id: string; rustdeskId: string; status: string }>();
...
function policyReceiptWhere(query: Request['query']): Prisma.StrategyApplyReceiptWhereInput {
  ...
  return {
    ...
    ...(status ? { status: status as 'PENDING' | 'APPLIED' | 'FAILED' } : {})
  };
}
...
function addressBookWhere(req: Request): Prisma.AddressBookWhereInput {
...
function connectionWhere(query: Request['query']): Prisma.ConnectionRecordWhereInput {
  const q = typeof query.q === 'string' ? query.q : undefined;
  const deviceId = typeof query.deviceId === 'string' ? query.deviceId : undefined;
...
function connectionAuditSummary(connection: {
  ...
  peerRustdeskId?: string | null;
  direction?: string | null;
  startedAt: Date;
  endedAt?: Date | null;
  device?: { id: string; rustdeskId: string; hostname?: string | null; username?: string | null } | null;
}) {
  return {
    id: connection.id,
...
function recordingWhere(query: Request['query']): Prisma.RecordingWhereInput {
  const q = typeof query.q === 'string' ? query.q : undefined;
  const status = typeof query.status === 'string' ? query.status : undefined;
  const deviceId = typeof query.deviceId === 'string' ? query.deviceId : undefined;
  return {
    ...(deviceId ? { deviceId } : {}),
    ...(status ? { status: status as 'UPLOADING' | 'COMPLETED' | 'REMOVED' | 'FAILED' } : {}),
    ...(q
      ? {
          OR: [
...
function validateRecordingTimeline(startedAt: Date | null | undefined, completedAt: Date | null | undefined) {
...
router.post('/recordings/upload', requirePermission('recordings.write'), upload.single('file'), async (req, res) => {
  ...
  let metadata: Prisma.InputJsonValue = { uploadedBy: 'admin', originalName: file.originalname };
...
router.get('/recordings/:id/download', requirePermission('recordings.read'), async (req, res) => {
  ...
  stream.once('error', async (error: StreamError) => {
    try {
      await auditLog({
        action: 'RECORD_DOWNLOAD',
        resource: 'recording',
...
router.delete('/recordings/:id', requirePermission('recordings.write'), async (req, res) => {
  ...
  let fileState: 'deleted' | 'missing' = 'deleted';
  try {
    await fs.unlink(recording.path);
  } catch (error) {
    const fsError = error as StreamError;
...
function auditLogWhere(query: Request['query']): Prisma.AuditLogWhereInput {
...
function isFutureDate(value: Date, now = new Date()) {
...
function serializeAddressBookPeer<T extends { password?: string | null }>(peer: T) {
