...
function extractClientToken(req: Request) {
  ...
  const body = req.body as { access_token?: unknown; token?: unknown } | undefined;
...
function assertClientDeviceAccess(req: Request, rustdeskId: string) {
...
function assertClientRecordAccess(req: Request, deviceId?: string | null) {
...
async function upsertClientDevice(req: Request, body: unknown, ipAddress?: string) {
...
function isDisabledDevice(device: { status: string }) {
  return device.status === 'DISABLED';
}

function recordingSize(value: string | number | bigint | undefined, fallback = 0n) {
...
function optionalRecordingSize(value: string | number | bigint | undefined) {
  if (value === undefined) return undefined;
  return recordingSize(value);
}

function recordingSafeName(filename: string) {
...
function validateClientRecordingStatusTransition(current: RecordingStatus, requested: RecordingStatus) {
...
function jsonValue(value: unknown) {
  return sanitizeAuditMetadata(value) as Prisma.InputJsonValue;
}
...
function connectionTimelineError(data: z.infer<typeof connectionSchema>, now = new Date()) {
  const startedAt = optionalDate(data.startedAt);
  const endedAt = optionalDate(data.endedAt);
  if (startedAt && isFutureDate(startedAt, now)) return 'Connection startedAt cannot be in the future';
  if (endedAt && isFutureDate(endedAt, now)) return 'Connection endedAt cannot be in the future';
  if (startedAt && endedAt && endedAt < startedAt) return 'Connection endedAt cannot be before startedAt';
  return null;
}

function uniqueStrategyIds(strategyIds: Array<string | undefined>) {
  const ids = [...new Set(strategyIds.filter((strategyId): strategyId is string => Boolean(strategyId)))];
  return ids.length > 0 ? ids : [undefined];
}

async function findDeviceForRecording(req: Request, input: { id?: unknown; rustdesk_id?: unknown; deviceId?: unknown }) {
  const rustdeskId = String(input.id ?? input.rustdesk_id ?? input.deviceId ?? '');
  if (!rustdeskId || rustdeskId === 'unknown') return null;
  assertClientDeviceAccess(req, rustdeskId);
  return prisma.device.findUnique({ where: { rustdeskId } });
}

async function findDeviceForClient(req: Request, input: { id?: unknown; rustdesk_id?: unknown; deviceId?: unknown }) {
  const rustdeskId = String(input.id ?? input.rustdesk_id ?? input.deviceId ?? '');
  if (!rustdeskId || rustdeskId === 'unknown') return null;
  assertClientDeviceAccess(req, rustdeskId);
  return prisma.device.findUnique({ where: { rustdeskId } });
...
async function validatePolicyReceiptStrategies(device: { id: string; groupId?: string | null }, strategyIds: Array<string | undefined>) {
  const explicitIds = [...new Set(strategyIds.filter((strategyId): strategyId is string => Boolean(strategyId)))];
...
async function findAddressBookUser(input: { userId?: string; username?: string; email?: string }) {
  if (input.userId) return prisma.user.findUnique({ where: { id: input.userId } });
  if (input.username) return prisma.user.findUnique({ where: { username: input.username } });
  if (input.email) return prisma.user.findUnique({ where: { email: input.email } });
  return null;
}

function requestedAddressBookUser(input: { userId?: string; username?: string; email?: string }) {
  return Boolean(input.userId || input.username || input.email);
}

async function requireAddressBookIdentityAuth(req: Request, res: Response, input: { userId?: string; username?: string; email?: string }) {
  if (!requestedAddressBookUser(input)) return true;
  if (req.clientAuth?.kind === 'global') return true;
...
function addressBookVisibility(userId?: string) {
  return {
    OR: [
      { shareRule: 'public' },
      ...(userId
...
function serializeClientAddressBookPeer<T extends { password?: string | null }>(peer: T) {
  return {
    ...peer,
...
function serializeClientAddressBook<T extends { peers?: Array<{ password?: string | null }> }>(book: T) {
  return {
    ...book,
...
function addressBookSyncSummary(books: Array<{
  id: string;
  guid: string;
  name: string;
  ownerId?: string | null;
...
function connectionFields(data: z.infer<typeof connectionSchema>) {
  return {
...
function connectionMetadata(
  ...
  event: 'start' | 'refresh' | 'end',
  data: Partial<z.infer<typeof connectionSchema>>,
...
function hasConnectionIdentity(fields: { connectionId?: number; peerRustdeskId?: string }) {
  return fields.connectionId !== undefined || Boolean(fields.peerRustdeskId);
}

function openConnectionWhere(deviceId: string, fields: { connectionId?: number; peerRustdeskId?: string; direction?: string }): Prisma.ConnectionRecordWhereInput {
  return {
    deviceId,
    endedAt: null,
...
async function findOpenConnections(deviceId: string, fields: { connectionId?: number; peerRustdeskId?: string; direction?: string }) {
  if (!hasConnectionIdentity(fields)) return [];
...
async function closeDuplicateOpenConnections(openConnections: Array<{ id: string; metadata: Prisma.JsonValue | null }>, canonicalId: string, endedAt = new Date()) {
...
async function createOrRefreshConnection(deviceId: string, data: z.infer<typeof connectionSchema>) {
...
async function saveRecordingFile(recordingId: string, file: UploadedFile, body: Record<string, unknown>) {
  const existing = await prisma.recording.findUnique({ where: { id: recordingId }, select: { status: true } });
  if (!existing) {
    return { status: 404, error: 'Recording upload not found' } as const;
  }
  if (existing.status !== 'UPLOADING') {
    return { status: 409, error: 'Recording is not accepting uploads', statusValue: existing.status } as const;
  }
...
async function saveRecordingChunk(recordingId: string, file: UploadedFile, body: Record<string, unknown>) {
  ...
  let metadata: Prisma.InputJsonValue = jsonValue({
  ...
  if (clientMetadata !== undefined) {
    metadata = jsonValue({ ...(metadata as object), clientMetadata });
  }
...
router.post('/sysinfo_ver', async (req, res) => {
  ...
  if (rustdeskId) assertClientDeviceAccess(req, rustdeskId);
...
router.post('/heartbeat', async (req, res) => {
  ...
  await prisma.deviceHeartbeat.create({
    data: {
      deviceId: device.id,
      conns: (req.body as { conns?: unknown }).conns as object | undefined,
      payload: req.body
    }
  });

  const conns = Array.isArray((req.body as { conns?: unknown }).conns) ? ((req.body as { conns: unknown[] }).conns) : [];
  const identifiableConns = conns.filter((conn) => {
    const data = conn as z.infer<typeof connectionSchema>;
    return hasConnectionIdentity(connectionFields(data)) && !connectionTimelineError(data);
  });
  await Promise.all(identifiableConns.map((conn) => createOrRefreshConnection(device.id, conn as z.infer<typeof connectionSchema>)));
...
router.get('/ab', async (req, res) => {
  ...
  if (!await requireAddressBookIdentityAuth(req, res, parsed.data)) return;
