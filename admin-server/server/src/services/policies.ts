...
export async function resolveDevicePolicy(rustdeskId: string) {
  ...
  const config = assignments.reduce<JsonObject>((merged, assignment) => ({
    ...merged,
    ...asObject(assignment.strategy.configOptions)
  }), {});
  const configSources = assignments.reduce<Record<string, { strategyId: string; strategyName: string; source: string; modifiedAt: number }>>((sources, assignment) => {
...
export async function resolveDevicePolicyById(deviceId: string) {
  const device = await prisma.device.findUnique({
    where: { id: deviceId },
...
export async function previewGroupPolicies(groupId: string) {
