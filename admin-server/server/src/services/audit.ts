...
export async function auditLog(input: AuditInput) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const data = {
      ...
      metadata: input.metadata === undefined ? undefined : (sanitizeAuditMetadata(input.metadata) as Prisma.InputJsonValue),
...
export function auditEntryHash(log: Parameters<typeof auditEntryPayload>[0]) {
  return crypto
    .createHash('sha256')
...
export function escapeCsvFormula(text: string) {
...
export async function verifyAuditChain(limit = 10000) {
  ...
  const issues: Array<{ id: string; createdAt: Date; reason: string }> = [];
