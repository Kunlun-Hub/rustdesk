import type { IdentityProvider, Prisma, User } from '@prisma/client';
...
export type ExternalProfile = {
  ...
  displayName?: string;
  avatar?: string;
  source: 'oidc' | 'wecom' | 'dingtalk_oauth2' | 'dingtalk_sns';
  rawProfile: Prisma.InputJsonValue;
};

export type ExternalUserLinkResult = {
  user: User;
  link: {
    action: 'existing_link' | 'linked_existing_user' | 'created_user';
    identityId: string;
...
export async function oidcConfiguration(provider: IdentityProvider) {
  ...
  return oidc.discovery(
    new URL(providerWithSecrets.issuerUrl),
    providerWithSecrets.clientId,
    {
...
export async function buildOidcAuthorizationUrl(provider: IdentityProvider, state: string, codeVerifier: string) {
...
export async function completeOidcLogin(provider: IdentityProvider, currentUrl: URL, codeVerifier: string): Promise<ExternalProfile> {
  const configuration = await oidcConfiguration(provider);
  ...
  let profile: Record<string, unknown> = claims as Record<string, unknown>;
...
async function fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const body = await response.json() as T & { errcode?: number; errmsg?: string; code?: string; message?: string };
...
export async function completeWeComLogin(provider: IdentityProvider, code: string): Promise<ExternalProfile> {
  const providerWithSecrets = decryptIdentityProviderSecrets(provider);
  ...
  const token = await fetchJson<{ access_token: string }>(
    `https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=${encodeURIComponent(providerWithSecrets.corpId)}&corpsecret=${encodeURIComponent(corpSecret)}`
  );
  const profile = await fetchJson<Record<string, unknown>>(
    `https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token=${encodeURIComponent(token.access_token)}&code=${encodeURIComponent(code)}`
  );
...
export async function completeDingTalkLogin(provider: IdentityProvider, code: string): Promise<ExternalProfile> {
  const providerWithSecrets = decryptIdentityProviderSecrets(provider);
  ...
  const token = await fetchJson<{ accessToken: string }>('https://api.dingtalk.com/v1.0/oauth2/userAccessToken', {
    method: 'POST',
  ...
  const profile = await fetchJson<Record<string, unknown>>('https://api.dingtalk.com/v1.0/contact/users/me', {
    method: 'GET',
...
export async function completeLegacyDingTalkSnsLogin(provider: IdentityProvider, code: string): Promise<ExternalProfile> {
  const providerWithSecrets = decryptIdentityProviderSecrets(provider);
  if (!providerWithSecrets.appKey || !providerWithSecrets.appSecret) {
    throw new Error('DingTalk provider is missing appKey or appSecret');
  }
  ...
  const signature = encodeURIComponent(
    crypto.createHmac('sha256', providerWithSecrets.appSecret).update(timestamp).digest('base64')
  );
  const profile = await fetchJson<{ user_info?: Record<string, unknown> }>(
    `https://oapi.dingtalk.com/sns/getuserinfo_bycode?accessKey=${encodeURIComponent(providerWithSecrets.appKey)}&timestamp=${timestamp}&signature=${signature}`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  return {
    ...
    source: 'dingtalk_sns',
    rawProfile: userInfo as Prisma.InputJsonValue
  };
}

function stableUsername(provider: IdentityProvider, profile: ExternalProfile) {
...
async function defaultExternalUserRoleCreate() {
  if (!config.EXTERNAL_USER_DEFAULT_ROLE) return undefined;
  const role = await prisma.role.findUnique({ where: { name: config.EXTERNAL_USER_DEFAULT_ROLE } });
  if (!role) return undefined;
  return { create: [{ roleId: role.id }] };
}

export async function findOrCreateExternalUser(provider: IdentityProvider, profile: ExternalProfile): Promise<ExternalUserLinkResult> {
  const safeRawProfile = sanitizeAuditMetadata(profile.rawProfile) as Prisma.InputJsonValue;
...
export function isScanProvider(provider: IdentityProvider) {
  return provider.type === AuthProviderType.WECOM || provider.type === AuthProviderType.DINGTALK;
}

export function scanLoginStart(provider: IdentityProvider, state: string) {
...
export async function completeScanLogin(provider: IdentityProvider, code: string): Promise<ExternalProfile> {
  if (provider.type === AuthProviderType.WECOM) {
