...
function deviceWhere(query) {
    ...
    return {
        ...(q
            ...
            : {}),
        ...(status === 'ONLINE' ? { online: true, status: { not: 'DISABLED' } } : {}),
        ...(status === 'OFFLINE' ? { online: false, status: { not: 'DISABLED' } } : {}),
        ...(status === 'DISABLED' ? { status: 'DISABLED' } : {}),
        ...(groupId ? { groupId } : {})
    };
}
function groupWhere(query) {
    const q = typeof query.q === 'string' ? query.q : undefined;
    return {
        ...(q
            ? {
...
async function findDuplicateDeviceIdentity(input, exceptId) {
    const checks = [];
    if (input.rustdeskId)
        checks.push({ rustdeskId: input.rustdeskId });
    if (input.uuid)
        checks.push({ uuid: input.uuid });
    if (checks.length === 0)
        return [];
    ...
    for (const device of duplicates) {
        if (input.rustdeskId && device.rustdeskId === input.rustdeskId)
            fields.add('rustdeskId');
        if (input.uuid && device.uuid === input.uuid)
            fields.add('uuid');
    }
    return [...fields].sort();
}
...
router.post('/devices', requirePermission('devices.write'), async (req, res) => {
    ...
    const device = await prisma.device.upsert({
        ...
        create: toDeviceData(parsed.data)
    });
...
router.post('/devices/bulk', requirePermission('devices.write'), async (req, res) => {
    ...
    if (parsed.data.groupId !== undefined)
        updates.groupId = parsed.data.groupId;
