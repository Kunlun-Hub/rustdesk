...
export async function runHealthChecks() {
    const [users, roles, permissions, devices, deviceGroups, strategies, strategyReceipts, addressBooks, connections, activeConnections, recordings, identityProviders, externalIdentityAccounts, permissionRows, superAdminRole, seededExternalRole, enabledProviders, externalRole, auditChain] = await Promise.all([
        prisma.user.count(),
        prisma.role.count(),
    ...
    const missingSeedData = [];
    if (users < 1)
        missingSeedData.push('users');
    if (roles < 1)
        missingSeedData.push('roles');
    if (permissions < 1)
        missingSeedData.push('permissions');
    if (strategies < 1)
        missingSeedData.push('strategies');
    if (addressBooks < 1)
        missingSeedData.push('addressBooks');
